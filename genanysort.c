...
int         Gen_length;         /*record the number of each record wanted to generator */
...
int buf_malloc(unsigned char *buf)
{
    if(Gen_length)
    {
        if(!(buf = (unsigned char *)malloc(Gen_length * sizeof(char))))
        {
            printf("error malloc\n");
            return -1;
        }
...
int buf_free(unsigned char *buf)
{
    if(!buf)
    {
        free(buf);
...
void gen_ascii_any(unsigned char *buf, rand_queue *rq)
{
    ...
    //temp = rand.hi8;
    rec_buf = buf + i * 10;
    for(i = 0; i < (Gen_length - 2) % 10; i++)
    {
        rec_buf[i] = (unsigned char)(' ' + (temp % 95));
        temp /= 95;
    }
...
int gen_block(sp_task_t t, void *unused)
{
	...
	//temp = rec_buf;
        ...
        if (Print_checksum)
        {
            temp16.lo8 = crc32(0, rec_buf, Gen_length);
            sum16 = add16(sum16, temp16);
        }
        if (!Skip_output)
            pfunc_write(t, 0, rec_buf, Gen_length);
        bump_queue(&rq);
    }
...
int main(int argc, char *argv[])
{
	...
	Gen_length = 100;
        ...
        {
            /*if(Gen == gen_ascii_rec)*/
                Gen = gen_ascii_any;
            Gen_length = atoi(argv[1] + 2);
            if(Gen_length < 10 || Gen_length > 100)
                usage();
        }
        else if (argv[1][1] == 's')
        {
    ...
    if (number_threads != 1)
    {
        ...
        ret = sp_start(&sp_gen, gen_block,
                       ...
                       BLK_RECS * Gen_length,          /* output buf size */
                       argv[2],                      /* file name */
                       number_threads);
        if (ret)
        {
...
#endif
    {
        ...
        for (j = 0; j < num_recs.lo8; j++)
        {
            ...
            if (!Skip_output)
                fwrite(rec_buf, Gen_length, 1, out);
            bump_queue(&rq);
        }
    }
